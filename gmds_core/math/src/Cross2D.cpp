...
/*-----------------------------------------------------------------*/
Cross2D::Cross2D(const Vector3d &ARef)
{
	Vector3d v({1., 0., 0.});
...
Cross2D::Cross2D(const Cross2D &AC)
{
	...
	if (m_known_vectors) m_vectors = AC.m_vectors;
}
/*-----------------------------------------------------------------*/
Vector3d
Cross2D::closestComponentVector(const Vector3d &AN) const
{
...
unsigned int
Cross2D::closestComponentVectorAsIndex(const Vector3d &AN) const
{
	std::vector<Vector3d> vecs = (m_known_vectors) ? m_vectors : componentVectors();
...
/*-----------------------------------------------------------------*/
bool
Cross2D::hasStoredComponentVectors() const
{
	return m_known_vectors;
}
/*-----------------------------------------------------------------*/
std::vector<Vector3d>
Cross2D::componentVectors() const
{
...
/*-----------------------------------------------------------------*/
void
Cross2D::computeComponentVectors()
{
	if (!m_known_vectors) {
...
/*-----------------------------------------------------------------*/
Vector3d
Cross2D::referenceVector() const
{
	return {cos(m_angle), sin(m_angle), 0.0};
}
/*-----------------------------------------------------------------*/
Cross2D
Cross2D::mean(const vector<Cross2D> &ACrosses, const vector<TCoord> &AWeights, const TInt ANbSteps)
{
	if (ACrosses.empty()) {
	...
	do {
		...
		TCoord pen_angle = 0.;
		for (const auto &ACrosse : ACrosses) {
			Vector3d vec_i = ACrosse.referenceVector();
			const TCoord pen_i = ref_vector.orientedAngle(vec_i);
			pen_angle += pen_i;     //*AWeights[i];
		}
		pen_angle /= static_cast<double>(ACrosses.size());
	...
	return Cross2D(ref_angle);
}

/*-----------------------------------------------------------------*/
Cross2D
Cross2D::meanNotMedian(const vector<Cross2D> &ACrosses, const vector<TCoord> &AWeights, const TInt ANbSteps)
{
	if (ACrosses.empty()) {
		throw GMDSException("The mean of zero 2D crosses is undefined.");
	}
	...
	return {center_vect, center_vect_second};
}
/*-----------------------------------------------------------------*/
Cross2D
Cross2D::mean(const Cross2D &AC1, const TCoord &AW1, const Cross2D &AC2, const TCoord &AW2)
{
...
/*-----------------------------------------------------------------*/
TCoord
Cross2D::angle(const Cross2D &AC) const
{
...
/*-----------------------------------------------------------------*/
int
Cross2D::index(const Cross2D &AC1, const Cross2D &AC2, const Cross2D &AC3)
{
	...
	return static_cast<int>(std::round((wij + wjk + wki) / Constants::PI2));
}
/*-----------------------------------------------------------------*/
ostream &
operator<<(ostream &AStr, const Cross2D &AC)
{
...
/*-----------------------------------------------------------------*/
Cross2D
operator+(const Cross2D &AC1, const Cross2D &AC2)
{
