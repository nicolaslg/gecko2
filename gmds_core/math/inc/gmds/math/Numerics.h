...
namespace gmds{
  ...
  namespace math{
      /*----------------------------------------------------------------------------*/
      bool  isZero(TCoord a, TCoord AEpsilon = Constants::EPSILON);
	 
	  bool isZero2ndMethod(TCoord a);

      bool areEquals(TCoord a, TCoord b, TCoord AEpsilon = Constants::EPSILON);

    /*------------------------------------------------------------------------*/
    /** \brief Compute AVal modulo AMod
     */
    TCoord modulo(TCoord AVal, TCoord AMod);
    /*------------------------------------------------------------------------*/
    /** \brief Compute AVal modulo 2xPI
     */
    TCoord modulo2PI(TCoord AVal);
    /*------------------------------------------------------------------------*/
    /** \brief Returns the min value between AV1, AV2 and AV3
     */
    TCoord min3(TCoord AV1, TCoord AV2, TCoord AV3);
    /*------------------------------------------------------------------------*/
     ...
     */
    bool  near(TCoord AV1, TCoord AV2, TCoord AEpsilon = Constants::EPSILON);
