...
FACManager::~FACManager()
{
	for (unsigned int i = 0; i < m_volumes.size(); i++)
		if (m_volumes[i] != nullptr) delete m_volumes[i];

	for (unsigned int i = 0; i < m_surfaces.size(); i++)
		if (m_surfaces[i] != nullptr) delete m_surfaces[i];

	for (unsigned int i = 0; i < m_curves.size(); i++)
		if (m_curves[i] != nullptr) delete m_curves[i];
...
FACManager::initFrom3DMesh(Mesh *AFromMesh)
{
	...
	for (auto f2s : faces_per_surface) {
		...
		FACSurface *s = new FACSurface(&m_mesh, f2s.second, surface_names[f2s.first]);
...
/*----------------------------------------------------------------------------*/
void
FACManager::initAndLinkFrom3DMesh(Mesh *AFromMesh, GeomMeshLinker *ALinker)
{
	...
	boundary_extractor.setMappings(&node_map_from_vol, &edge_map_from_vol, &face_map_from_vol, &node_map_from_surf, &edge_map_from_surf, &face_map_from_surf);
	boundary_extractor.execute();
...
FACManager::updateFromMesh()
{
	...
	// faces to surfaces
	gmds::Variable<FACSurface *> *surfaceAssociation = m_mesh.newVariable<FACSurface *, gmds::GMDS_FACE>("surfaceAssociation");
	{
		...
		for (; itf != m_mesh.groups_end<gmds::Face>(); itf++) {
			...
			FACSurface *s = new FACSurface(&m_mesh, surf_faces, current_surface->name());
			m_surfaces.push_back(s);
		...
		if (1 != nodeIDs.size()) {
			...
			for (unsigned int iNode = 0; iNode < nodeIDs.size(); iNode++) {
				...
				for (unsigned int iEdge = 0; iEdge < edges.size(); iEdge++) {
					...
					if ((*markCurveNodes)[edgeNodes[0].id()] == 1 && (*markCurveNodes)[edgeNodes[1].id()] == 1) {
						...
						if (2 < edgeFaces.size()) {
							...
							if ((*surfaceAssociation)[edgeFaces[0].id()] != (*surfaceAssociation)[edgeFaces[1].id()]) {
								hasOneSurface = false;
								break;
							}
						}
				...
				for (unsigned int iEdge = 0; iEdge < edges.size(); iEdge++) {
					...
					if (edges[iEdge].id() != current_edge_id) {
						...
						if ((*markCurveNodes)[edgeNodes[0].id()] == 1 && (*markCurveNodes)[edgeNodes[1].id()] == 1) {
							...
							if (hasOneSurface || ((*surfaceAssociation)[edgeFaces[0].id()] != (*surfaceAssociation)[edgeFaces[1].id()])) {
			...
			FACCurve *c = new FACCurve(&m_mesh, p1, p2, orderedNodes, edgeIDs_path, current_cloud->name());
...
/*----------------------------------------------------------------------------*/
TInt
FACManager::getNbCurves() const
{
	return m_curves.size();
}
/*----------------------------------------------------------------------------*/
TInt
FACManager::getNbSurfaces() const
{
	return m_surfaces.size();
}
/*----------------------------------------------------------------------------*/
TInt
FACManager::getNbVolumes() const
{
	return m_volumes.size();
}
/*----------------------------------------------------------------------------*/
void
FACManager::getVolumes(std::vector<GeomVolume *> &volumes) const
{
	volumes.clear();
...
/*----------------------------------------------------------------------------*/
void
FACManager::getCurves(std::vector<GeomCurve *> &curves) const
{
	curves.clear();
...
/*----------------------------------------------------------------------------*/
GeomCurve *
FACManager::getCurve(const gmds::TInt AID)
{
	for (auto c : m_curves) {
...
/*----------------------------------------------------------------------------*/
GeomSurface *
FACManager::getSurface(const gmds::TInt AID)
{
	for (auto s : m_surfaces) {
...
/*----------------------------------------------------------------------------*/
GeomVolume *
FACManager::getVolume(const gmds::TInt AID)
{
	for (auto v : m_volumes) {
		if (v->id() == AID) return v;
	}
	return nullptr;
}
/*----------------------------------------------------------------------------*/
int
FACManager::getCommonCurve(GeomPoint *AP1, GeomPoint *AP2) const
{
	...
	std::vector<TCellID> e1 = n1.getIDs<Edge>();
	std::vector<TCellID> e2 = n2.getIDs<Edge>();
...
FACManager::getCommonSurfaces(gmds::cad::GeomPoint *AP1, gmds::cad::GeomPoint *AP2) const
{
	...
	Variable<int> *face_on_surf = m_mesh.getVariable<int, GMDS_FACE>("on_surface");

	std::vector<TCellID> face_ids1 = n1.getIDs<Face>();
...
/*----------------------------------------------------------------------------*/
int
FACManager::getCommonSurface(GeomCurve *AC1, GeomCurve *AC2) const
{
