...
namespace gmds{
    ...
    public:
        ...
        void resize(TInt ASize){
            throw GMDSException("No resize for this TabCellID specialization, size param: "+std::to_string(ASize));
        }
    ...
    /*----------------------------------------------------------------------------*/
    template<> struct  TabCellID<size_undef> {
     ...
     */
    std::vector<TCellID>  getCommonBut(const std::vector<TCellID>& AS1,
     ...
     * 		   occurrences in ASet
     */
    std::vector<TCellID>   keepFilter(const std::vector<TCellID>& ASet,
            const TInt ANb);
    /*----------------------------------------------------------------------------*/
    /** \class VirtualEdge
     *
