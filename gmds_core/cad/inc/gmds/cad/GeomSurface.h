 ...
 */
/*----------------------------------------------------------------------------*/
class GeomSurface : public GeomEntity
{
 ...
 public:
	...
	/** \brief  Constructor
	 */
	explicit GeomSurface(const std::string &AName = "Unknown surface") : GeomEntity(AName) {}
	 ...
	 */
	virtual void computeNormal(const math::Point &AP, math::Vector3d &AV) const = 0;
	 ...
	 * @return points that are adjacent to this surface
	 */
	virtual std::vector<GeomPoint *> &points() = 0;

	/**@brief Accessor to the adjacent curves. Warning, there is no
	 *  assumption about the ordering
	 * @return curves that are adjacent to this surface
	 */
	 ...
	 * @return loops
	 */
	virtual std::vector<std::vector<GeomCurve *> > loops() = 0;
	/**@brief Accessor to the adjacent volumes. Warning, there is no
	 *  assumption about the ordering
	 * @return volumes that are adjacent to this surface
	 */
