...
Blocking::extract_boundary(std::set<TCellID> &ANodeIds, std::set<TCellID> &AEdgeIds, std::set<TCellID> &AFaceIds) {
	...
	AEdgeIds.clear();
	AFaceIds.clear();
	for (auto it = m_cmap.attributes<2>().begin(), itend = m_cmap.attributes<2>().end(); it != itend; ++it) {
...
Blocking::get_faces_of_block(const Blocking::Block AB) {
	...
	faces[0] = m_cmap.attribute<2>(d1);
	faces[1] = m_cmap.attribute<2>(m_cmap.beta<2, 1, 1, 2>(d1));
	faces[2] = m_cmap.attribute<2>(m_cmap.beta<2>(d1));
	faces[3] = m_cmap.attribute<2>(m_cmap.beta<1, 1, 2>(d1));
	faces[4] = m_cmap.attribute<2>(m_cmap.beta<1, 2>(d1));
	faces[5] = m_cmap.attribute<2>(m_cmap.beta<1, 1, 1, 2>(d1));
	return faces;
}
...
Blocking::get_all_blocks() {
	...
	for (auto attr: m_cmap.attributes<3>()) {
		blocks.push_back(m_cmap.attribute<3>(attr.dart()));
	}
	return blocks;
}
...
Blocking::get_all_faces() {
	std::vector<Blocking::Face> faces;
	for (auto attr: m_cmap.attributes<2>()) {
		faces.push_back(m_cmap.attribute<2>(attr.dart()));
	}
	return faces;
}
...
Blocking::get_all_id_faces() {
	const auto faces = get_all_faces();
	std::vector<TCellID> ids;
...
Blocking::get_all_id_blocks() {
	const auto blocks = get_all_blocks();
	std::vector<TCellID> ids;
...
Blocking::get_faces_of_node(const Node AN) {
	...
	Dart3 d = AN->dart();
	for (auto it = m_cmap.one_dart_per_incident_cell<2, 0>(d).begin(), itend = m_cmap.one_dart_per_incident_cell<2,
				     0>(d).end(); it != itend; ++it) {
...
Blocking::get_edges_of_face(const Face AF) {
	std::vector<Edge> edges;
	Dart3 d = AF->dart();
	for (auto it = m_cmap.one_dart_per_incident_cell<1, 2>(d).begin(), itend = m_cmap.one_dart_per_incident_cell<1,
				     2>(d).end(); it != itend; ++it) {
		edges.push_back(m_cmap.attribute<1>(it));
	}
	return edges;
...
Blocking::get_blocks_of_edge(const Edge AE) {
	std::vector<Block> blocks;
	Dart3 d = AE->dart();
	for (auto it = m_cmap.one_dart_per_incident_cell<3, 1>(d).begin(), itend = m_cmap.one_dart_per_incident_cell<3,
				     1>(d).end(); it != itend; ++it) {
		blocks.push_back(m_cmap.attribute<3>(it));
	}
	return blocks;
...
Blocking::get_blocks_of_face(const Face AF) {
	std::vector<Block> blocks;
	Dart3 d = AF->dart();
	for (auto it = m_cmap.one_dart_per_incident_cell<3, 2>(d).begin(), itend = m_cmap.one_dart_per_incident_cell<3,
				     2>(d).end(); it != itend; ++it) {
		blocks.push_back(m_cmap.attribute<3>(it));
	}
	return blocks;
...
Blocking::get_all_edges() {
	...
	for (auto e: m_cmap.attributes<1>()) {
		edges.push_back(m_cmap.attribute<1>(e.dart()));
	}
	return edges;
}
...
Blocking::get_all_nodes() {
	...
	for (auto n: m_cmap.attributes<0>()) {
		nodes.push_back(m_cmap.attribute<0>(n.dart()));
	}
	return nodes;
}
...
Blocking::get_edges_of_block(const Blocking::Block AB) {
	...
	edges[0] = m_cmap.attribute<1>(d);
	edges[1] = m_cmap.attribute<1>(m_cmap.beta<1, 1, 2>(d));
	edges[2] = m_cmap.attribute<1>(m_cmap.beta<1, 1, 2, 1, 1, 2>(d));
	edges[3] = m_cmap.attribute<1>(m_cmap.beta<2, 1, 1>(d));
	d = m_cmap.beta<1>(d);
	edges[4] = m_cmap.attribute<1>(d);
	edges[5] = m_cmap.attribute<1>(m_cmap.beta<1, 1, 2>(d));
	edges[6] = m_cmap.attribute<1>(m_cmap.beta<1, 1, 2, 1, 1, 2>(d));
	edges[7] = m_cmap.attribute<1>(m_cmap.beta<2, 1, 1>(d));
	d = m_cmap.beta<1>(d);
	d = m_cmap.beta<2, 1>(d);
	edges[8] = m_cmap.attribute<1>(d);
	edges[9] = m_cmap.attribute<1>(m_cmap.beta<1, 1, 2>(d));
	edges[10] = m_cmap.attribute<1>(m_cmap.beta<1, 1, 2, 1, 1, 2>(d));
	edges[11] = m_cmap.attribute<1>(m_cmap.beta<2, 1, 1>(d));
...
Blocking::get_nodes_of_block(const Blocking::Block AB) {
	...
	std::vector<Blocking::Node> nodes;
	nodes.resize(8);
	nodes[0] = m_cmap.attribute<0>(d1);
	nodes[1] = m_cmap.attribute<0>(m_cmap.beta<1>(d1));
	nodes[2] = m_cmap.attribute<0>(m_cmap.beta<1, 1>(d1));
	nodes[3] = m_cmap.attribute<0>(m_cmap.beta<1, 1, 1>(d1));
	nodes[4] = m_cmap.attribute<0>(m_cmap.beta<2, 1, 1>(d1));
	nodes[5] = m_cmap.attribute<0>(m_cmap.beta<1, 2, 1, 1>(d1));
	nodes[6] = m_cmap.attribute<0>(m_cmap.beta<1, 1, 2, 1, 1>(d1));
	nodes[7] = m_cmap.attribute<0>(m_cmap.beta<1, 1, 1, 2, 1, 1>(d1));
...
Blocking::get_nodes_of_face(const Blocking::Face AF) {
	const Dart3 d1 = AF->dart();
	std::vector<Blocking::Node> nodes;
	Dart3 d = d1;
	do {
		nodes.push_back(m_cmap.attribute<0>(d));
		d = m_cmap.beta<1>(d);
	} while (d != d1);
	return nodes;
}
...
Blocking::get_nodes_of_edge(const Blocking::Edge AE) {
	...
	nodes.resize(2);
	Dart3 d = AE->dart();
	auto d_node = m_cmap.attribute<0>(d);
	auto d0_node = m_cmap.attribute<0>(m_cmap.beta<2>(d));
...
Blocking::create_block(const math::Point &AP1,
	...
	Dart3 d1 = m_cmap.make_combinatorial_hexahedron();

	// 0D attribute
	m_cmap.set_attribute<0>(d1, create_node(4, NullID, AP1));
	m_cmap.set_attribute<0>(m_cmap.beta<1>(d1), create_node(4, NullID, AP2));
	m_cmap.set_attribute<0>(m_cmap.beta<1, 1>(d1), create_node(4, NullID, AP3));
	m_cmap.set_attribute<0>(m_cmap.beta<1, 1, 1>(d1), create_node(4, NullID, AP4));

	m_cmap.set_attribute<0>(m_cmap.beta<2, 1, 1>(d1), create_node(4, NullID, AP5));
	m_cmap.set_attribute<0>(m_cmap.beta<1, 2, 1, 1>(d1), create_node(4, NullID, AP6));
	m_cmap.set_attribute<0>(m_cmap.beta<1, 1, 2, 1, 1>(d1), create_node(4, NullID, AP7));
	m_cmap.set_attribute<0>(m_cmap.beta<1, 1, 1, 2, 1, 1>(d1), create_node(4, NullID, AP8));
	// go through all the edges
	for (auto it = m_cmap.one_dart_per_incident_cell<1, 3>(d1).begin(), itend = m_cmap.one_dart_per_incident_cell<1,
				     3>(d1).end(); it != itend; ++it) {
		m_cmap.set_attribute<1>(it, create_edge(4, NullID));
	}

	// go through all the faces
	for (auto it = m_cmap.one_dart_per_incident_cell<2, 3>(d1).begin(), itend = m_cmap.one_dart_per_incident_cell<2,
				     3>(d1).end(); it != itend; ++it) {
		m_cmap.set_attribute<2>(it, create_face(4, NullID));
	}
	Block b = create_block(4, NullID);
	m_cmap.set_attribute<3>(d1, b);
	return b;
}
...
Blocking::get_node_info(const int ANodeId) {
	for (auto it = m_cmap.attributes<0>().begin(), itend = m_cmap.attributes<0>().end(); it != itend; ++it) {
...
Blocking::get_edge_info(const int AEdgeId) {
	for (auto it = m_cmap.attributes<1>().begin(), itend = m_cmap.attributes<1>().end(); it != itend; ++it) {
...
Blocking::get_block_info(const int ABlockId) {
	for (auto it = m_cmap.attributes<3>().begin(), itend = m_cmap.attributes<3>().end(); it != itend; ++it) {
...
Blocking::info() const {
	...
	m_cmap.display_characteristics(mess);
	mess << ", validity=" << (is_valid_topology() ? "true" : "false");
	mess << "\nOd-attributes: ";
	for (auto at: m_cmap.attributes<0>()) {
		mess << "[" << at.info().point << ",(dim:" << at.info().geom_dim << ",id:" << at.info().geom_id << ")]; ";
	}
	mess << "\n1d-attributes: ";
	for (auto at: m_cmap.attributes<1>()) {
		mess << "(dim:" << at.info().geom_dim << ",id:" << at.info().geom_id << "); ";
	}
	mess << "\n2d-attributes: ";
	for (auto at: m_cmap.attributes<2>()) {
		mess << "(dim:" << at.info().geom_dim << ",id:" << at.info().geom_id << "); ";
	}
	mess << "\n3d-attributes: ";
	for (auto at: m_cmap.attributes<3>()) {
		mess << "(dim:" << at.info().geom_dim << ",id:" << at.info().geom_id << "); ";
	}
	return mess.str();
}
...
Blocking::convert_to_mesh(Mesh &AMesh) {
	...
	if (!model.has(N) || !model.has(E) || !model.has(F) || !model.has(R) || !model.has(E2N) || !model.has(F2N) || !model
	    .has(R2N))
	...
	// edges
	for (auto it = m_cmap.attributes<1>().begin(), itend = m_cmap.attributes<1>().end(); it != itend; ++it) {
		auto att = m_cmap.info_of_attribute<1>(it);
	...
	// faces
	for (auto it = m_cmap.attributes<2>().begin(), itend = m_cmap.attributes<2>().end(); it != itend; ++it) {
		auto att = m_cmap.info_of_attribute<2>(it);
		std::vector<Node> cell_nodes = get_nodes_of_face(it);
		if (cell_nodes.size() != 4) {
			std::cerr << "ERROR: Wrong number of nodes for exporting a face: " << cell_nodes.size() << std::endl;
			continue;
	...
	// blocks
	for (auto it = m_cmap.attributes<3>().begin(), itend = m_cmap.attributes<3>().end(); it != itend; ++it) {
		auto att = m_cmap.info_of_attribute<3>(it);
...
Blocking::init_from_mesh(Mesh &AMesh) {
	...
	if (!model.has(N) || !model.has(R) || !model.has(R2N))
		throw GMDSException(
			"Wrong mesh model for mesh->block->mesh conversion");
	...
	for (auto r_id: AMesh.regions()) {
		...
		if (r.type() == GMDS_HEX) {
			...
			Block b = create_block(ns[0].point(), ns[1].point(), ns[2].point(), ns[3].point(), ns[4].point(),
			                       ns[5].point(), ns[6].point(), ns[7].point());
	...
	// now we glue blocks;
	for (auto it_r = m_cmap.attributes<3>().begin(), it_rend = m_cmap.attributes<3>().end(); it_r != it_rend; ++it_r) {
		std::vector<Face> cell_faces = get_faces_of_block(it_r);
		for (auto f: cell_faces) {
			Dart3 d = f->dart();
			if (m_cmap.is_free<3>(d)) {
				...
				for (auto it_f = m_cmap.attributes<2>().begin(), it_fend = m_cmap.attributes<2>().end(); it_f != it_fend
				     ; ++it_f) {
					auto att = m_cmap.info_of_attribute<2>(it_f);
					Dart3 d2 = it_f->dart();
					if (d2 != d && m_cmap.is_free<3>(d2)) {
						...
						if (!one_missing) {
							...
							TCellID ref_id = n2n[m_cmap.attribute<0>(d)->info().topo_id];
							TCellID ref_id_0 = n2n[m_cmap.attribute<0>(m_cmap.beta<2>(d))->info().topo_id];

							// starting from d2, we move to find the one that correspond to ref_id
							Dart3 d_glue = d2;
							while (n2n[m_cmap.attribute<0>(d_glue)->info().topo_id] != ref_id) {
								d_glue = m_cmap.beta<1>(d_glue);
							}
							if (n2n[m_cmap.attribute<0>(m_cmap.beta<0>(d_glue))->info().topo_id] == ref_id_0) {
								m_cmap.sew<3>(d_glue, d);
							} else {
								m_cmap.sew<3>(m_cmap.beta<1>(d_glue), d);
							}
						}
					}
				}
...
Blocking::get_all_sheet_darts(const Edge AE, std::vector<Dart3> &ADarts) {
	...
	auto edge_mark = m_cmap.get_new_mark();
	...
	auto opp_dart = m_cmap.beta<2>(first_dart);
	if (m_cmap.attribute<0>(first_dart)->info().topo_id > m_cmap.attribute<0>(opp_dart)->info().topo_id)
		first_dart = opp_dart;

	front.push_back(first_dart);
	// the current dart belongs to the final set of darts
	ADarts.push_back(first_dart);
	// we mark all the dart of the initial edge to avoid to traverse it twice
	m_cmap.mark_cell<1>(first_dart, edge_mark);
	...
	while (!front.empty()) {
		...
		// we traverse the darts of the orbit<2,3> starting from d, and getting the dart oriented in the same
		// direction as d
		auto cur_dart = d;
		auto reach_an_end = false;
		std::vector<Dart3> stack;
		stack.push_back(d);
		do {
			auto next_dart = m_cmap.beta<2, 3>(cur_dart);
			if (m_cmap.is_free(next_dart, 3)) {
				reach_an_end = true;
			} else {
				cur_dart = next_dart;
				stack.push_back(cur_dart);
			}
		} while (cur_dart != d && !reach_an_end);

		if (reach_an_end) {
			//means we are on a boundary edge, and we must turn in the opposite direction now
			reach_an_end = false;
			do {
				auto next_dart = m_cmap.beta<3, 2>(cur_dart);
				if (m_cmap.is_free(next_dart, 3)) {
					reach_an_end = true;
				} else {
					cur_dart = next_dart;
		...
		for (auto cur_d: stack) {
			//we get the 3 parallel edges in the hex
			Dart3 d_next[3] = {
				m_cmap.beta<1, 1, 2>(cur_d),
				m_cmap.beta<2, 1, 1>(cur_d),
				m_cmap.beta<1, 1, 2, 1, 1, 2>(cur_d)
				};
			for (auto i = 0; i < 3; i++) {
				if (!m_cmap.is_marked(d_next[i], edge_mark)) {
					...
					front.push_back(d_next[i]);
					m_cmap.mark_cell<1>(d_next[i], edge_mark);
					// We also add it to the set of darts to return
					ADarts.push_back(d_next[i]);
				}
			}
		}

		// we traverse the darts of the orbit<2,3> starting from d
		/*for (auto it(m_cmap.darts_of_orbit<2, 3>(d).begin()), itend(m_cmap.darts_of_orbit<2, 3>(d).end()); it != itend;
		     ++it) {
			auto d_next_edge = m_cmap.beta<1, 1, 2>(it);
			if (!m_cmap.is_marked(d_next_edge, edge_mark)) {
				// it means that the edge containing the dart d_next_edge has not been traversed already.
				// We mark the dart of the corresponding edge, and we add it to the front
				front.push_back(d_next_edge);
				m_cmap.mark_cell<1>(d_next_edge, edge_mark);
				// We also add it to the set of darts to return
...
Blocking::get_all_chord_blocks(const Face AF) {
	...
	auto block_mark = m_cmap.get_new_mark();

	std::vector<Dart3> face_darts;
	get_all_chord_darts(AF, face_darts);
	for (auto d: face_darts) {
		if (!m_cmap.is_marked(d, block_mark)) {
			bls.push_back(m_cmap.attribute<3>(d));
			m_cmap.mark_cell<3>(d, block_mark);
		}
		if (!m_cmap.is_marked(m_cmap.beta<3>(d), block_mark)) {
			bls.push_back(m_cmap.attribute<3>(m_cmap.beta<3>(d)));
			m_cmap.mark_cell<3>(m_cmap.beta<3>(d), block_mark);
		}
	}
...
Blocking::get_all_chord_darts(const Face AF, std::vector<Dart3> &ADarts) {
	...
	auto face_mark = m_cmap.get_new_mark();

	std::vector<Dart3> front;
	front.push_back(AF->dart());
	if (!m_cmap.is_free<3>(AF->dart())) front.push_back(m_cmap.beta<3>(AF->dart()));

	// the current dart belongs to the final set of darts
	ADarts.push_back(AF->dart());
	// we mark all the dart of the initial face to avoid to traverse it twice
	m_cmap.mark_cell<2>(AF->dart(), face_mark);
	...
	while (!front.empty()) {
		...
		auto d_next_face = m_cmap.beta<2, 1, 1, 2, 3>(d);
		if (!m_cmap.is_marked(d_next_face, face_mark)) {
			// it means that the edge containing the dart d_next_edge has not been traversed already.
			// We mark the dart of the corresponding edge, and we add it to the front
			front.push_back(d_next_face);
			m_cmap.mark_cell<2>(d_next_face, face_mark);
			// We also add it to the set of darts to return
			ADarts.push_back(d_next_face);
			if (!m_cmap.is_free<3>(d_next_face)) front.push_back(m_cmap.beta<3>(d_next_face));
		}
	}
	// We must unmark all the marked edges. As we stored one dart per edge, it is straightforward
...
Blocking::cut_sheet(const Edge AE, const double AParam) {
	...
	auto mark_edge_darts = m_cmap.get_new_mark();

	//we store the new nodes
	std::vector<Node> new_nodes;
	for (auto d: sheet_darts) {
		Dart3 d2 = m_cmap.beta<2>(d);
		math::Point pa = m_cmap.attribute<0>(d)->info().point;
		math::Point pb = m_cmap.attribute<0>(d2)->info().point;
		auto edge_att = m_cmap.attribute<1>(d)->info();

		Dart3 d_edge = m_cmap.insert_cell_0_in_cell_1(d);
		auto darts_23 = m_cmap.darts_of_orbit<2, 3>(d_edge);
		for (auto d_edge_23 = darts_23.begin(); d_edge_23 != darts_23.end(); d_edge_23++) {
			in_edge_darts.push_back(d_edge_23);
			m_cmap.mark(d_edge_23, mark_edge_darts);
		}
		...
		auto new_n = create_node(edge_att.geom_dim, edge_att.geom_id, pc);
		m_cmap.set_attribute<0>(d_edge, new_n);
		new_nodes.push_back(new_n);
	}

	// then we cut faces by inserting an edge
	auto mark_done = m_cmap.get_new_mark();
	std::vector<Dart3> in_face_darts;
	for (auto d: in_edge_darts) {
		if (!m_cmap.is_marked(d, mark_done)) {
			...
			auto node_of_d = m_cmap.attribute<0>(d);
			bool found = std::find(new_nodes.begin(), new_nodes.end(), node_of_d) != new_nodes.end();
			if (!found) {
				d = m_cmap.beta<1>(d);
			}


			Dart3 d2 = m_cmap.beta<1, 1, 1>(d);
			auto face_att = m_cmap.attribute<2>(d)->info();

			Dart3 d_face = m_cmap.insert_cell_1_in_cell_2(d, d2);

			m_cmap.set_attribute<1>(d_face, create_edge(face_att.geom_dim, face_att.geom_id));

			in_face_darts.push_back(d_face);
			m_cmap.mark_cell<2>(d, mark_done);
			m_cmap.mark_cell<2>(d2, mark_done);
		}
	}
	...
	for (auto d: in_face_darts) {
		if (!m_cmap.is_marked(d, mark_done)) {
			...
			do {
				...
				m_cmap.mark(current_dart, mark_done);
				m_cmap.mark(m_cmap.beta<2>(current_dart), mark_done);
				current_dart = m_cmap.beta<1, 2, 1>(current_dart);
			} while (current_dart != d);
			auto face_att = m_cmap.attribute<2>(d)->info();
			Dart3 d_block = m_cmap.insert_cell_2_in_cell_3(loop_darts.begin(), loop_darts.end());
			// a face is created by splitting a block, it is classified on a 3-dimensional geom cell
			m_cmap.set_attribute<2>(d_block, create_face(4, NullID));
		}
	}
...
// 	auto mark_face = m_cmap.get_new_mark();
// 	for (auto f: AFaces) {
// 		m_cmap.mark_cell<2>(f->dart(), mark_face);
// 	}
//
...
// 				if (m_cmap.is_marked(f_of_e->dart(), mark_face)) {
...
//
// 	for (auto f: AFaces) {
// 		m_cmap.unmark_cell<2>(f->dart(), mark_face);
// 	}
// 	m_cmap.free_mark(mark_face);
//
// 	return !found_error;
// }
...
/*----------------------------------------------------------------------------*/
void
Blocking::mark_half_face(gecko::cblock::Dart3 ADart, int AMark) {
	Dart3 d = ADart;
	do {
		m_cmap.mark(d, AMark);
		d = m_cmap.beta<1>(d);
...
// 	Dart3 blue_dart;
// 	for (auto it(m_cmap.darts_of_orbit<1, 2, 3>(d).begin()), itend(m_cmap.darts_of_orbit<1, 2, 3>(d).end());
// 	     it != itend && !found_blue_dart; ++it) {
// 		if (m_cmap.is_marked(it, AMark)) {
...
// 	for (auto it = m_cmap.one_dart_per_incident_cell<2, 0>(blue_dart).begin(), itend = m_cmap.one_dart_per_incident_cell
// 					     <2, 0>(blue_dart).end(); it != itend;
// 	     ++it) {
// 		auto fi = m_cmap.attribute<2>(it);
// 		if (m_cmap.is_marked(it, AMark) || m_cmap.is_marked(m_cmap.beta<3>(it), AMark)) {
// 			nb_incident_blue_faces++;
// 		}
// 	}
// 	auto nb_incident_blue_blocks = 0;
// 	auto mark_block = m_cmap.get_new_mark();
// 	for (auto it = m_cmap.one_dart_per_incident_cell<3, 0>(blue_dart).begin(), itend = m_cmap.one_dart_per_incident_cell
// 					     <3, 0>(blue_dart).end(); it != itend;
// 	     ++it) {
// 		// We have one dart of the block (the dart belongs to the vertex too
// 		auto found_blue_dart_in_block = false;
// 		m_cmap.mark_cell<3>(it, mark_block);
// 		for (auto it2 = m_cmap.one_dart_per_incident_cell<2, 0>(it).begin(), it2end = m_cmap.one_dart_per_incident_cell<
// 					     2, 0>(it).end(); it2 != it2end; ++it2) {
// 			// the provided dart does not necessary belong to the block
// 			if (m_cmap.is_marked(it2, AMark) && m_cmap.is_marked(it2, mark_block)) {
// 				found_blue_dart_in_block = true;
// 			} else if (m_cmap.is_marked(m_cmap.beta<3>(it2), AMark) && m_cmap.
// 			           is_marked(m_cmap.beta<3>(it2), mark_block)) {
// 				found_blue_dart_in_block = true;
// 			}
// 		}
...
// 		}
// 		m_cmap.unmark_cell<3>(it, mark_block);
// 	}
// 	m_cmap.free_mark(mark_block);
// 	int cell_topo_dim = 0;
// 	int cell_topo_id = 0;
// 	int cell_geom_dim = 0;
// 	int cell_geom_id = 0;
...
// 		Dart3 edge_dart = m_cmap.beta<2, 1>(blue_dart);
// 		auto edge_info = m_cmap.attribute<1>(edge_dart);
...
// 		// slide along a  face
// 		// the blue dart belongs to the node, the face and block
// 		Dart3 face_dart = m_cmap.beta<2>(blue_dart);
// 		if (m_cmap.is_marked(face_dart, AMark)) {
// 			// we pick the third face adjacent to the node and that belongs to the block
// 			face_dart = m_cmap.beta<1, 2>(blue_dart);
// 		}
// 		auto face_info = m_cmap.attribute<2>(face_dart);
...
// 		// get inside a block
// 		auto block_info = m_cmap.attribute<3>(blue_dart);
...
// 	auto mark_blue_side = m_cmap.get_new_mark();
// 	auto mark_red_side = m_cmap.get_new_mark();
//
...
// 		for (auto f: AFaces) {
// 			m_cmap.mark_cell<2>(f->dart(), mark_blue_side);
// 		}
// 	} else {
...
// 			for (auto f: AFaces) {
// 				auto d = f->dart();
// 				if (!m_cmap.is_marked(d, mark_blue_side) && !m_cmap.is_marked(d, mark_red_side)) {
...
// 						Dart3 edge_dart = m_cmap.beta<2>(current_dart);
// 						auto found_blue = false;
// 						auto found_red = false;
// 						do {
// 							if (m_cmap.is_marked(edge_dart, mark_blue_side)) {
// 								found_blue = true;
// 							} else if (m_cmap.is_marked(edge_dart, mark_red_side)) {
// 								found_red = true;
// 							} else {
// 								// we go to the next dart
// 								edge_dart = m_cmap.beta<3, 2>(edge_dart);
// 							}
// 						} while (edge_dart != current_dart && edge_dart != m_cmap.beta<2>(current_dart) && !found_blue
// 						         && !found_red);
// 						// color the face?
// 						if (found_blue) {
...
// 							if (!m_cmap.is_free<3>(current_dart)) {
// 								mark_half_face(m_cmap.beta<3>(current_dart), mark_red_side);
// 							}
// 						} else if (found_red) {
// 							color_current_face = true;
...
// 							if (!m_cmap.is_free<3>(current_dart)) {
// 								mark_half_face(m_cmap.beta<3>(current_dart), mark_blue_side);
// 							}
// 						}
// 						// we go to the next dart
// 						current_dart = m_cmap.beta<0, 1>(current_dart);
...
// 			Dart3 d = f->dart();
// 			if (m_cmap.is_free<3>(d)) {
// 				if (m_cmap.is_marked(d, mark_blue_side))
// 					nb_only_blue++;
// 				else if (m_cmap.is_marked(d, mark_red_side))
// 					nb_only_red++;
// 				else
// 					throw GMDSException("Error during blue-red coloring of face");
// 			} else if ((m_cmap.is_marked(d, mark_blue_side) && m_cmap.is_marked(m_cmap.beta<3>(d), mark_red_side))
// 			           || (m_cmap.is_marked(d, mark_red_side) && m_cmap.is_marked(m_cmap.beta<3>(d), mark_blue_side))) {
// 				nb_blue_and_red++;
// 			} else
// 				throw GMDSException("Error during blue-red coloring of face");
// 		}
...
// 			m_cmap.unmark_all(mark_face);
// 			m_cmap.unmark_all(mark_blue_side);
// 			m_cmap.unmark_all(mark_red_side);
// 			m_cmap.free_mark(mark_face);
// 			m_cmap.free_mark(mark_blue_side);
// 			m_cmap.free_mark(mark_red_side);
// 			return false;
// 		}
// 	}
//
...
// 		for (CMap3::Dart_of_orbit_range<1, 2, 3>::iterator it(m_cmap.darts_of_orbit<1, 2, 3>(n->dart()).begin()),
// 				     itend(m_cmap.darts_of_orbit<1, 2, 3>(n->dart()).end());
// 		     it != itend; ++it) {
...
// 		auto [n1_tdim, n1_tid, n1_gdim, n1_gid] = pillow_node_data[m_cmap.beta<0>(d_edge)];
...
// 			edge_id = n0_gid;
// 		}
// 		for (CMap3::Dart_of_orbit_range<0, 2, 3>::iterator it(m_cmap.darts_of_orbit<0, 2, 3>(d_edge).begin()),
// 				     itend(m_cmap.darts_of_orbit<0, 2, 3>(d_edge).end());
// 		     it != itend; ++it) {
...
// 		if (m_cmap.is_marked(df, mark_blue_side))
// 			blue_darts.push_back(df);
// 		else
// 			blue_darts.push_back(m_cmap.beta<3>(df));
// 	}
...
// 		} else {
// 			was_sewed[d] = true;
// 			prev_3_dart[d] = m_cmap.beta<3>(d);
// 			m_cmap.unsew<3>(d);
// 		}
// 	}
//
// 	auto mark_already_merged_node = m_cmap.get_new_mark();
// 	auto mark_already_merged_edge = m_cmap.get_new_mark();
// 	for (auto df: blue_darts) {
// 		// We are going to duplicate this face
// 		Face f = m_cmap.attribute<2>(df);
// 		// df is blue
// 		std::vector<Node> nodes_of_f = get_nodes_of_face(f);
...
// 		math::Point df_point = m_cmap.attribute<0>(df)->info().point;
// 		double dist_min = df_point.distance2(m_cmap.attribute<0>(db)->info().point);
// 		Dart3 db_min = db;
// 		Dart3 db_cur = m_cmap.beta<0, 1>(db);
// 		while (db_cur != db) {
// 			double dist_cur = df_point.distance2(m_cmap.attribute<0>(db_cur)->info().point);
// 			if (dist_cur < dist_min) {
// 				dist_min = dist_cur;
// 				db_min = db_cur;
// 			}
// 			db_cur = m_cmap.beta<0, 1>(db_cur);
// 		}
// 		// We must now to check if df must be sewed to db_min or a<1>(db_min)
// 		math::Point pnt_df0 = m_cmap.attribute<0>(m_cmap.beta<0>(df))->info().point;
// 		math::Point pnt_db_min0 = m_cmap.attribute<0>(m_cmap.beta<0>(db_min))->info().point;
// 		math::Point pnt_db_min10 = m_cmap.attribute<0>(m_cmap.beta<1, 0>(db_min))->info().point;
// 		if (pnt_db_min10.distance2(pnt_df0) < pnt_db_min0.distance2(pnt_df0)) {
// 			// switch the edge s
// 			db_min = m_cmap.beta<1>(db_min);
// 		}
//
// 		// we copy the face attribute
// 		Dart3 d_opp_face = m_cmap.beta<2, 1, 0, 1, 2>(db_min);
// 		if (was_sewed[df]) {
// 			m_cmap.sew<3>(d_opp_face, prev_3_dart[df]);
// 		} else {
// 			m_cmap.attribute<2>(d_opp_face)->info().geom_dim = f->info().geom_dim;
// 			m_cmap.attribute<2>(d_opp_face)->info().geom_id = f->info().geom_id;
// 		}
// 		m_cmap.sew<3>(df, db_min);
// 		m_cmap.attribute<2>(df)->info().geom_dim = 3;
// 		m_cmap.attribute<2>(df)->info().geom_id = NullID;
//
// 		// after sewing, we change classification info
// 		Dart3 d = df;
// 		do {
// 			auto att_node_d = m_cmap.attribute<0>(d);
// 			auto att_edge_d = m_cmap.attribute<1>(d);
//
// 			if (!m_cmap.is_marked(d, mark_already_merged_node)) {
// 				m_cmap.mark_cell<0>(d, mark_already_merged_node);
//
// 				// the opposite node keep the previous embedding
// 				Dart3 d_opp = m_cmap.beta<3, 2, 1, 0, 1, 2>(d);
// 				m_cmap.attribute<0>(d_opp)->info().geom_id = att_node_d->info().geom_id;
// 				m_cmap.attribute<0>(d_opp)->info().geom_dim = att_node_d->info().geom_dim;
//
// 				// the current node is embedded onto the detected geom feature
// 				auto [tdim, tid, gdim, gid] = pillow_node_data[d];
// 				m_cmap.attribute<0>(d)->info().geom_dim = gdim;
// 				m_cmap.attribute<0>(d)->info().geom_id = gid;
//
// 				// the edge connected the two nodes is classified onto the same feature
// 				Dart3 d_edge = m_cmap.beta<3, 2, 1>(d);
// 				m_cmap.attribute<1>(d_edge)->info().geom_dim = gdim;
// 				m_cmap.attribute<1>(d_edge)->info().geom_id = gid;
// 			}
//
// 			if (m_cmap.is_marked(d, mark_already_merged_edge)) {
...
// 				Dart3 d3 = m_cmap.beta<2>(d);
// 				bool found = false;
// 				do {
// 					if (m_cmap.is_marked(d3, mark_blue_side)) {
// 						found = true;
// 					} else {
// 						d3 = m_cmap.beta<3, 2>(d3);
// 					}
// 				} while (!found);
// 				m_cmap.sew<3>(m_cmap.beta<3, 2>(d), m_cmap.beta<3, 2>(d3));
// 			} else {
// 				m_cmap.mark_cell<1>(d, mark_already_merged_edge);
// 				auto [e_geom_dim, e_geom_id] = pillow_edge_data[d];
//
// 				// the opposite edge keep the previous embedding
// 				Dart3 d_opp = m_cmap.beta<3, 2, 1, 0, 1, 2>(d);
// 				m_cmap.attribute<1>(d_opp)->info().geom_id = att_edge_d->info().geom_id;
// 				m_cmap.attribute<1>(d_opp)->info().geom_dim = att_edge_d->info().geom_dim;
//
// 				// the current edge is embedded onto the detected geom feature
// 				//  std::cout<<"Edge "<<m_cmap.attribute<1>(d)->info().topo_id<<" on: ("<<e_geom_dim<<", "<<e_geom_id<<")"<<std::endl;
// 				m_cmap.attribute<1>(d)->info().geom_dim = e_geom_dim;
// 				m_cmap.attribute<1>(d)->info().geom_id = e_geom_id;
//
// 				// the face connected the two edge is classified onto the same feature
// 				Dart3 d_face = m_cmap.beta<3, 2>(d);
// 				m_cmap.attribute<2>(d_face)->info().geom_dim = e_geom_dim;
// 				m_cmap.attribute<2>(d_face)->info().geom_id = e_geom_id;
// 			}
// 			d = m_cmap.beta<0, 1>(d);
// 		} while (d != df);
// 	}
...
// 	m_cmap.unmark_all(mark_blue_side);
// 	m_cmap.unmark_all(mark_red_side);
// 	m_cmap.unmark_all(mark_already_merged_node);
// 	m_cmap.unmark_all(mark_already_merged_edge);
//
// 	m_cmap.free_mark(mark_face);
// 	m_cmap.free_mark(mark_blue_side);
// 	m_cmap.free_mark(mark_red_side);
// 	m_cmap.free_mark(mark_already_merged_node);
// 	m_cmap.free_mark(mark_already_merged_edge);
...
// 		Node ni = m_cmap.attribute<0>(chord_darts[i]);
// 		Node nj = m_cmap.attribute<0>(chord_darts_opp[i]);
...
// 		to_sew.push_back(std::make_pair(m_cmap.beta<2, 3>(di), m_cmap.beta<2, 3>(dj)));
// 		to_sew.push_back(std::make_pair(m_cmap.beta<1, 2, 3>(di), m_cmap.beta<1, 2, 3>(dj)));
// 		if (!m_cmap.is_free<3>(di)) {
// 			to_sew.push_back(std::make_pair(m_cmap.beta<3, 2, 3>(di), m_cmap.beta<3, 2, 3>(dj)));
// 			to_sew.push_back(std::make_pair(m_cmap.beta<3, 1, 2, 3>(di), m_cmap.beta<3, 1, 2, 3>(dj)));
// 		}
// 	}
...
// 	for (auto dd : to_sew) {
// 		if (m_cmap.is_free<3>(dd.first)) {
// 			m_cmap.sew<3>(dd.first, dd.second);
// 		}
// 	}
// 	return true;
// }
